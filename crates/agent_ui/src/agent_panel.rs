    ...
    cx.observe_new(
        |workspace: &mut Workspace, _window, _cx: &mut Context<Workspace>| {
            workspace
                ...
                .register_action(|_workspace, _: &OpenAgentDiff, _window, _cx| {
                    // Terry: no editor-style AgentDiff review surface.
                })
...
impl AgentPanel {
    ...
    pub fn toggle_focus(
        ...
        // Bottom Agent button opens the LLM chat panel. Only toggle closed when
        // Agent is already the active left-dock panel and focused.
        let agent_is_active_panel = workspace
            .left_dock()
            .read(cx)
            .active_panel()
            .is_some_and(|p| p.persistent_name() == Self::persistent_name());
        let agent_focused = panel.focus_handle(cx).contains_focused(window, cx);

        if agent_is_active_panel && agent_focused {
    ...
    fn new_thread_with_workspace(
        ...
        // Terry: Agent panel is the LLM chat surface (terminal/MCP via tools).
        // Embedded "Terminal Thread" is opt-in from the new-entry menu only.
    ...
    pub fn should_create_terminal_for_new_entry(&self, _cx: &App) -> bool {
        // Terry: opening the Agent panel always starts a chat thread so the
        // user can talk to the model (which then uses terminal/MCP tools).
        false
    }
    ...
    /// Work dirs for Agent chat: prefer the active terminal folder so the
    /// model/MCP tools operate in that directory without requiring Open Project.
    fn agent_work_dirs(&self, cx: &App) -> PathList {
        if let Some(cwd) = self.active_terminal_cwd(cx) {
            return PathList::new(&[cwd]);
        }
        if let Some(cwd) = self.default_terminal_working_directory(cx) {
            return PathList::new(&[cwd]);
        }
        self.project.read(cx).default_path_list(cx)
    }
    ...
    fn external_thread(
        ...
        let thread = self.create_agent_thread_with_server(
            ...
            None,
            resume_thread_id,
            work_dirs.or_else(|| Some(self.agent_work_dirs(cx))),
            title,
            initial_content,
            None,
            source,
    ...
    fn update_thread_work_dirs(&self, cx: &mut Context<Self>) {
        let new_work_dirs = self.agent_work_dirs(cx);
    ...
    fn create_agent_thread_inner(
        ...
        let workspace = self.workspace.clone();
        let project = self.project.clone();
        let work_dirs = Some(work_dirs.unwrap_or_else(|| self.agent_work_dirs(cx)));
...
fn agent_panel_dock_position(_cx: &App) -> DockPosition {
    // Terry: agent dialog lives in the left dock with terminal/file lists.
    DockPosition::Left
}
...
impl Panel for AgentPanel {
    ...
    fn set_position(&mut self, _position: DockPosition, _: &mut Window, cx: &mut Context<Self>) {
        // Terry: keep agent on the left; ignore requests to move it.
        settings::update_settings_file(self.fs.clone(), cx, move |settings, _| {
            settings
                .agent
                .get_or_insert_default()
                .set_dock(DockPosition::Left.into());
        });
    }
...
impl AgentPanel {
    /// Terry: the Agent dock panel is always the LLM chat UI (terminal/MCP tools
    /// run in the background). Never auto-open an embedded terminal here.
    /// Context is the active terminal's cwd — no Open Project required.
    fn ensure_chat_on_activate(&mut self, window: &mut Window, cx: &mut Context<Self>) {
    ...
    fn should_render_new_user_onboarding(&mut self, _cx: &mut Context<Self>) -> bool {
        // Terry: skip Zed cloud/account onboarding.
        false
    }
    ...
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let content = v_flex()
            ...
            .map(|parent| match self.visible_surface() {
                VisibleSurface::Uninitialized if self.is_active => {
                    cx.defer_in(window, |this, window, cx| {
                        this.ensure_chat_on_activate(window, cx);
                    });
                    parent
