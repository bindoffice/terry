...
use crate::graphics::{Image, ImageState};
...
pub struct Term<T> {
    ...
    /// Terminal graphics state (kitty protocol / sixel images).
    images: ImageState,

    /// Lines where shell prompts started (`OSC 133;A`/`F`), most recent last.
    ///
    /// Used by the UI to jump to the previous prompt.
...
pub struct Config {
    ...
    /// Whether terminal graphics (kitty protocol / sixel) are enabled.
...
impl Default for Config {
    fn default() -> Self {
        Self {
            ...
            max_image_memory: crate::graphics::DEFAULT_MAX_IMAGE_MEMORY,
        }
    }
}
...
impl<T> Term<T> {
        ...
        T: EventListener,
    {
        ...
        self.images.set_max_memory(self.config.max_image_memory);
        if !self.config.graphics_enabled {
            self.images.clear();
        }
    ...
    /// Access to the terminal graphics state.
    pub fn image_state(&self) -> &ImageState {
        &self.images
    }

    /// Mutable access to the terminal graphics state.
    pub fn image_state_mut(&mut self) -> &mut ImageState {
        &mut self.images
    }

    /// Whether terminal graphics are enabled.
    pub fn graphics_enabled(&self) -> bool {
        self.config.graphics_enabled
    }

    /// Update the pixel size of a grid cell for graphics sizing.
    pub fn set_image_cell_size(&mut self, width: f32, height: f32) {
        self.images.set_cell_size(width, height);
    }

    /// Drop image placements no longer referenced by any grid cell.
    pub fn gc_images(&mut self) {
        // Walk both the active and the inactive (alternate screen) grid, so
        // placements in either buffer survive.
        self.images.gc(&self.grid);
        self.images.gc(&self.inactive_grid);
    }

    /// Whether an image GC pass is needed.
    pub fn images_need_gc(&self) -> bool {
        self.images.needs_gc()
    }
    ...
    pub fn resize<S: Dimensions>(&mut self, size: S) {
        ...
        // Resizing can drop grid rows, orphaning image placements.
        self.images.set_needs_gc();
    ...
    fn write_at_cursor(&mut self, c: char) {
        ...
        // Writing over an image top-left cell orphans its placement.
        if let Some(id) = cursor_cell.image() {
            self.images.remove_placement(id);
        }
...
impl<T: EventListener> Handler for Term<T> {
    ...
    /// Handle an APC string, used by the kitty graphics protocol.
    #[inline]
    fn apc_dispatch(&mut self, bytes: &[u8]) {
        ...
        let mut respond = |response: String| {
            event_proxy.send_event(Event::PtyWrite(response));
        };
        images.handle_apc(grid, &mut respond, bytes);
    }

    /// DCS hook; only sixel (`q`) is intercepted for graphics.
    #[inline]
    fn hook(
        &mut self,
        ...
        if action == 'q' && self.config.graphics_enabled {
            self.images.sixel_hook(params);
        }
    }

    /// Feed a byte to an active DCS string.
    #[inline]
    fn put(&mut self, byte: u8) {
        self.images.sixel_put(byte);
    }

    /// Finish a DCS string.
    #[inline]
    fn unhook(&mut self) {
        self.images.sixel_unhook(&mut self.grid);
    }
    ...
    fn identify_terminal(&mut self, intermediate: Option<char>) {
        match intermediate {
            None => {
                ...
                // kitty-style DA1 response: `CSI ? 62 ; version ; flags c`.
                // The first parameter is the kitty DA1 marker (62), the second
                // the terminal version, the third a bitmask of supported
                // features (see [`DA1_FLAGS`]).
                let version = version_number(env!("CARGO_PKG_VERSION"));
                let text = format!("\x1b[?62;{version};{}c", DA1_FLAGS);
    ...
    fn set_notification(
        ...
        trace!("Desktop notification: title={title:?} body={body:?} progress={progress:?}");
        self.event_proxy
            .send_event(Event::Notification(title, body, progress));
    }

    /// Track the working directory reported via `OSC 7;file://...`.
    #[inline]
    fn set_cwd(&mut self, cwd: Option<String>) {
        let Some(uri) = cwd else {
            return;
        };
        match parse_cwd_uri(&uri) {
            Some(path) => {
                trace!("Current working directory: {}", path.display());
                self.event_proxy.send_event(Event::Cwd(path));
            }
            None => debug!("Ignoring malformed OSC 7 working directory URI: {uri}"),
        }
    }

    /// Handle OSC 133 shell integration markers.
    #[inline]
    fn shell_integration(&mut self, command: char, payload: Option<String>) {
        trace!("Shell integration: command={command} payload={payload:?}");
        match command {
            // Prompt start. If a previous prompt is still open, close it first.
            'A' => {
                self.finish_active_prompt();
                self.active_prompt_mark = Some(self.grid.cursor.point);
            }
            // Prompt end.
            'B' => self.finish_active_prompt(),
            // Mark the entire prompt (start and end at the current position).
            'F' => {
                self.finish_active_prompt();
...
///
/// Handles both `file:///path` (no host) and `file://host/path` forms,
/// percent-decoding the path component. Returns `None` for non-`file://`
/// URIs or URIs without a path.
fn parse_cwd_uri(uri: &str) -> Option<PathBuf> {
    // RFC 8089: `file://` followed by an optional authority (host) then path.
    let rest = uri.strip_prefix("file://")?;

    // `file://host/path`: split the authority at the first `/`. `file:///path`
    // already starts with `/`, so the whole remainder is the path.
...
/// Decode `%XX` escape sequences, leaving all other bytes untouched.
...
/// Feature bitmask sent in the kitty-style primary device attributes response
/// (`CSI ? 62 ; version ; flags c`). Each bit advertises a capability of the
/// terminal, mirroring the flags kitty defines in its `da1` response:
///
/// - 1      cursor colors (OSC 10/11/12)
/// - 16     mouse support
/// - 32     kitty keyboard protocol
/// - 64     sync updates
/// - 128    clickable hyperlinks (OSC 8)
/// - 256    read clipboard (OSC 52)
/// - 512    shell integration (OSC 133)
/// - 4096   graphics protocol
/// - 16384  notifications with progress (OSC 9)
const DA1_FLAGS: usize = 1 | 16 | 32 | 64 | 128 | 256 | 512 | 4096 | 16384;

impl<T> Term<T> {
    /// Lines where shell prompts started (`OSC 133;A`/`F`), oldest first.
    ///
    /// Points use grid coordinates; combine with the display offset to find the
    /// prompt's position on screen.
    pub fn prompt_marks(&self) -> impl Iterator<Item = &Point> {
        self.prompt_marks.iter()
    }
    ...
    fn finish_active_prompt(&mut self) {
        if let Some(mark) = self.active_prompt_mark.take() {
            self.push_prompt_mark(mark);
        }
    }

    /// Append a prompt mark, keeping the list bounded.
    fn push_prompt_mark(&mut self, mark: Point) {
...
pub struct RenderableContent<'a> {
    ...
    pub mode: TermMode,
    /// Images placed on the visible grid, sorted by z-index.
    pub images: Vec<RenderableImage>,
}

/// An image placement visible on the grid.
#[derive(Debug, Clone)]
...
impl<'a> RenderableContent<'a> {
    fn new<T>(term: &'a Term<T>) -> Self {
        // Collect visible image placements (top-left cells carrying an image
        // reference). The top-left must be within the visible window, matching
        // the cells yielded by `display_iter`.
        let images = term
            .grid()
            .display_iter()
            .filter(|indexed| indexed.cell.flags.contains(Flags::IMAGE))
            .filter_map(|indexed| {
                let placement_id = indexed.cell.image()?;
                let placement = term.images.placements.get(&placement_id)?;
                let image = term.images.images.get(&placement.image)?.clone();
...
mod tests {
    ...
    /// Listener that records every event emitted by the `Term`.
    ...
    #[test]
    fn osc52_clipboard_store_emits_event_when_allowed() {
        ...
        // Write base64-encoded text into the clipboard.
        processor.advance(&mut term, b"\x1b]52;c;aGVsbG8=\x1b\\");

        let events = events.lock().unwrap();
        assert_eq!(events.len(), 1);
        match &events[0] {
            Event::ClipboardStore(ClipboardType::Clipboard, text) => assert_eq!(text, "hello"),
            other => panic!("unexpected event: {other:?}"),
        }
    }

    #[test]
    fn osc52_clipboard_load_emits_event_when_allowed() {
        let events = Arc::new(Mutex::new(Vec::new()));
        let mut config = Config::default();
        config.osc52 = Osc52::CopyPaste;
        let mut term = Term::new(
        ...
        // A "?" payload requests reading the clipboard; BEL and ESC \ both terminate.
        processor.advance(&mut term, b"\x1b]52;c;?\x07");
        processor.advance(&mut term, b"\x1b]52;p;?\x1b\\");
        ...
        match &events[1] {
            Event::ClipboardLoad(ClipboardType::Selection, formatter) => {
                assert_eq!(formatter("hello"), "\x1b]52;p;aGVsbG8=\x1b\\");
            }
            other => panic!("unexpected event: {other:?}"),
        }
    }
    ...
    #[test]
    fn osc52_clipboard_load_denied_for_only_copy_config() {
        // The default config only permits writes (`Osc52::OnlyCopy`), so loads are dropped.
        let events = Arc::new(Mutex::new(Vec::new()));
        let mut term = Term::new(
            Config::default(),
            &TermSize::new(80, 24),
        ...
        processor.advance(&mut term, b"\x1b]52;c;?\x1b\\");
    ...
    #[test]
    fn osc9_notification_emits_event() {
        let events = Arc::new(Mutex::new(Vec::new()));
        let mut term = Term::new(
            Config::default(),
            &TermSize::new(80, 24),
        ...
        // Plain iTerm2 style: the whole payload is the title.
        processor.advance(&mut term, b"\x1b]9;Build finished\x07");
        // kitty extended form with title/body/progress, ST terminator.
        processor.advance(&mut term, b"\x1b]9;4;Build;Tests passed;50\x1b\\");

        let events = events.lock().unwrap();
        assert_eq!(events.len(), 2);
        match &events[0] {
            Event::Notification(title, body, progress) => {
                assert_eq!(title.as_deref(), Some("Build finished"));
    ...
    #[test]
    fn osc133_prompt_marks_recorded() {
        let mut term = Term::new(
            Config::default(),
            &TermSize::new(80, 24),
        ...
        // Prompt starts at (0, 0) and ends after `hello\n`.
        processor.advance(&mut term, b"\x1b]133;A\x07");
        processor.advance(&mut term, b"hello\n");
        processor.advance(&mut term, b"\x1b]133;B\x07");
        assert_eq!(
    ...
    #[test]
    fn da1_response_reports_kitty_features() {
        let events = Arc::new(Mutex::new(Vec::new()));
        let mut term = Term::new(
            Config::default(),
            &TermSize::new(80, 24),
        ...
        match &events[0] {
            Event::PtyWrite(response) => {
                let version = version_number(env!("CARGO_PKG_VERSION"));
                assert_eq!(response, &format!("\x1b[?62;{version};{DA1_FLAGS}c"));
            }
            other => panic!("unexpected event: {other:?}"),
        }
    }
    ...
    fn osc7_cwd_emits_event() {
        ...
        // No host, percent-encoded path.
        processor.advance(&mut term, b"\x1b]7;file:///home/user%20name\x07");
        // With host, BEL terminator.
        processor.advance(&mut term, b"\x1b]7;file://ssh-host/srv/www\x1b\\");
        // Non-file URI and missing path are both ignored.
        processor.advance(&mut term, b"\x1b]7;http://example.com/x\x07");
        processor.advance(&mut term, b"\x1b]7;file://host\x07");

        let events = events.lock().unwrap();
        assert_eq!(events.len(), 2);
        match &events[0] {
            Event::Cwd(path) => assert_eq!(path, &PathBuf::from("/home/user name")),
            other => panic!("unexpected event: {other:?}"),
        }
        match &events[1] {
            Event::Cwd(path) => assert_eq!(path, &PathBuf::from("/srv/www")),
            other => panic!("unexpected event: {other:?}"),
        }
    }

    #[test]
    fn parse_cwd_uri_handles_rfc8089_forms() {
        assert_eq!(
            parse_cwd_uri("file:///home/user"),
            Some(PathBuf::from("/home/user"))
        );
        assert_eq!(
            parse_cwd_uri("file://host/home/user"),
            Some(PathBuf::from("/home/user"))
        );
        assert_eq!(
            parse_cwd_uri("file:///home/user%20name"),
            Some(PathBuf::from("/home/user name"))
        );
        assert_eq!(parse_cwd_uri("file://host"), None);
        assert_eq!(parse_cwd_uri("https://example.com/x"), None);
        assert_eq!(parse_cwd_uri("file:///"), None);
    }
}
