...
impl<const OSC_RAW_BUF_SIZE: usize> Parser<OSC_RAW_BUF_SIZE> {
    ...
    /// Advance the parser state while inside an APC (Application Program
    /// Command, `ESC _`) string.
    ///
    /// Bytes are buffered and dispatched via [`Perform::apc_dispatch`] when the
    /// string is terminated by ST (`ESC \\` or `0x9C`) or BEL. Unlike OSC
    /// strings, the APC payload is opaque binary data (e.g. raw pixels for the
    /// kitty graphics protocol), so C0 control bytes are preserved.
    #[inline(always)]
    fn advance_apc_string<P: Perform>(&mut self, performer: &mut P, byte: u8) {
        match byte {
            ...
            0x1B => {
                // Dispatch at the ESC of an `ESC \\` terminator, mirroring how
                // OSC strings are ended in `advance_osc_string`.
                self.apc_end(performer);
                self.reset_params();
                self.state = State::Escape
            }
...
pub trait Perform {
    ...
    ///
    /// The APC payload (the bytes between `ESC _` and the string terminator)
    /// is passed to the performer. This is used by the kitty graphics
    /// protocol.
