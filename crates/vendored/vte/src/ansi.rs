...
pub trait Handler {
    ...
    /// OSC to show a desktop notification (kitty/iTerm2 `OSC 9`).
    ///
    /// `title`/`body` are the notification text, `progress` is an optional
    /// progress percentage (`Some(p)` with `p` in 0..=100, or `Some(-1)` for
    /// indeterminate progress).
    fn set_notification(
        &mut self,
        _title: Option<String>,
        _body: Option<String>,
        _progress: Option<i64>,
    ) {
    }

    /// OSC to report the current working directory (`OSC 7;file://...`).
    ///
    /// The value is the raw `file://` URI; `None` indicates the directory was
    /// explicitly unset. The handler is responsible for URI/percent decoding.
    fn set_cwd(&mut self, _: Option<String>) {}

    /// OSC 133 shell integration (prompt/command marking).
    ///
    /// `command` is one of `A` (prompt start), `B` (prompt end), `C` (command
    /// start), `D` (command end), `E` (run command on current line) or `F`
    /// (mark the full prompt). `payload` carries optional metadata (e.g. the
    /// `T=<text>` from `OSC 133;A;T=...`).
    fn shell_integration(&mut self, _command: char, _payload: Option<String>) {}
    ...
    /// Invoked when a DCS string is started.
    fn hook(&mut self, _params: &Params, _intermediates: &[u8], _ignore: bool, _action: char) {}

    /// Pass bytes as part of a DCS string.
    fn put(&mut self, _byte: u8) {}

    /// Called when a DCS string is terminated.
    fn unhook(&mut self) {}

    /// Dispatch an application program command (APC).
    ///
    /// Receives the raw payload between `ESC _` and the string terminator.
    ...
    T: Timeout,
{
    ...
    fn hook(&mut self, params: &Params, intermediates: &[u8], ignore: bool, action: char) {
        self.handler.hook(params, intermediates, ignore, action);
    }
    ...
    fn osc_dispatch(&mut self, params: &[&[u8]], bell_terminated: bool) {
        ...
        match params[0] {
            ...
            // Report the current working directory (OSC 7).
            b"7" => {
                let cwd = params[1..]
                    .iter()
                    .flat_map(|x| str::from_utf8(x))
                    .collect::<Vec<&str>>()
                    .join(";");
                self.handler.set_cwd((!cwd.is_empty()).then_some(cwd));
                return;
            }

            // Show desktop notification (kitty/iTerm2).
            b"9" => {
                // `OSC 9;title` (iTerm2) shows a notification whose payload is
                // the title. kitty also defines `OSC 9;4;title;body;progress`
                // where the fourth parameter is an optional progress indicator
                // (0..=100, or -1 for indeterminate progress).
                let title = || {
                    params[1..]
                        .iter()
                        .flat_map(|x| str::from_utf8(x))
                ...
                if params.len() >= 3 && params[1] == b"4" {
                    ...
                    self.handler.set_notification(title, body, progress);
                } else {
                    self.handler.set_notification(Some(title()), None, None);
                }
                return;
            }

            // Shell integration (kitty/fish/WezTerm `OSC 133`).
            b"133" => {
